...
public class Gaussian {
    ...
    public static double inverseCDF(double y) {
        //return inverseCDFRecursive(y, 0.00000001, -8, 8);
        return inverseCDFIterative(y, 0.00000001, -8, 8);
    }

    // bisection search
    // private static double inverseCDFRecursive(double y, double delta, double lo, double hi) {
    //     double mid = lo + (hi - lo) / 2;
    //     if (hi - lo < delta) return mid;
    //     if (cdf(mid) > y) return inverseCDFRecursive(y, delta, lo, mid);
    //     else              return inverseCDFRecursive(y, delta, mid, hi);
    // }

    private static double inverseCDFIterative(double y, double delta, double lo, double hi) {
        double mid = 0;
        while (hi - lo >= delta) {
            mid = lo + (hi - lo)/2;
            if (cdf(mid) > y) hi = mid;
